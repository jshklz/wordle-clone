...
GFX_DrawText(GFX_Text* text, GFX_Renderer* renderer)
{
    ...
    // Fixed advance positioning (Wordle-style grid layout)
    if (text->fixed_advance_x > 0) {
        r32 glyph_w = quad.x1 - quad.x0;
        r32 cell_center_x = text->pos.x + i * text->fixed_advance_x + text->fixed_advance_x / 2.0f;
        r32 dx = cell_center_x - (glyph_w / 2.0f) - quad.x0;
        quad.x0 += dx;
        quad.x1 += dx;
    } else {
