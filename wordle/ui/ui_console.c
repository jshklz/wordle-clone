...
int visible_lines = console->foreground.size.h / console->font.size;
if (console->line.text_used > visible_lines) {
    console->scrollbar.button.size.h = (console->scrollbar.base.size.h * visible_lines) / console->line.text_used;
} else {
    console->scrollbar.button.size.h = console->scrollbar.base.size.h;
}
